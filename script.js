...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ── Contact form: build a mailto link ── */
  ...
  if (form) {
    form.addEventListener('submit', (e) => {
      ...
      const subject = form.subject.value.trim() || 'Hello from your website';
      const message = form.message.value.trim();

      const body = `${message}\n\n— ${name} (${email})`;
      const mailto = `mailto:aliasherif164@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
